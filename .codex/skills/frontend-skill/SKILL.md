...
- Protected app areas live in `src/app/(protected)/member`, `src/app/(protected)/owner`, and `src/app/(protected)/staff`
- Shared global styles live in `src/app/globals.css`
- Feature-scoped CSS currently also lives in `src/app/`, especially `src/app/routine-creator.css` and `src/app/routine-execution.css`
- Feature components live in `src/components/feature/`
- Layout shell components live in `src/components/layout/`
- Shared primitives live in `src/components/ui/`
- Server actions live in `src/actions/`
- Business logic and helpers live in `src/lib/`

When applying this skill in this repo:

- Prefer editing existing feature entry points such as `src/components/feature/workout-session-editor.tsx`, `src/components/feature/weekly-plan-editor.tsx`, or `src/components/feature/routine-builder.tsx` before inventing new top-level structures.
- Keep shared UI behavior in `src/components/ui/` and route-specific presentation in feature components or route-level CSS.
- If a page already uses a dedicated stylesheet like `src/app/routine-execution.css`, extend that stylesheet instead of introducing scattered inline utility styling.
- Preserve the current app-shell structure around `src/components/layout/app-shell-client.tsx` and `src/components/layout/sidebar.tsx` when changing protected-area navigation or mobile menu behavior.
